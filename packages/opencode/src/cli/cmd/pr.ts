...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found ${process.env.CODY_PRO === "0" ? "opencode" : "Cody Pro"} session: ${sessionUrl}`)
          UI.println(`Importing session...`)

          const importResult = yield* Effect.promise(() =>
            Process.text([process.env.CODY_PRO === "0" ? "opencode" : "cody-pro", "import", sessionUrl], {
              nothrow: true,
            }),
          )
    ...
    UI.println()
    UI.println(`Starting ${process.env.CODY_PRO === "0" ? "opencode" : "Cody Pro"}...`)
    UI.println()

    const opencodeArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn([process.env.CODY_PRO === "0" ? "opencode" : "cody-pro", ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0)
      return yield* Effect.die(
        new Error(`${process.env.CODY_PRO === "0" ? "opencode" : "Cody Pro"} exited with code ${code}`),
      )
  }),
})
